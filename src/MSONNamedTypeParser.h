    ...
    : public SignatureSectionProcessorBase<mson::NamedType>
{
    ...
    static MarkdownNodeIterator finalizeSignature(
        ...
        if (subject[0] != '`' &&
            RegexMatch(out.node.name.symbol.literal, MSONReservedCharsRegex)) {
            ...
            out.report.warnings.push_back(
                Warning("please escape the name of the data structure using "
                        "backticks since it contains MSON reserved characters",
                    FormattingWarning,
                    sourceMap));
        }
    ...
    static MarkdownNodeIterator processNestedSection(
        ...
        return SectionProcessor<mson::ValueMember>::processNestedMembers<
