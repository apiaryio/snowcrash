...
MarkdownNodeIterator SectionProcessor<mson::TypeSection>::processNestedSection(
        ...
        case MSONOneOfSectionType: {
            if (parentSectionType != MSONPropertyMembersSectionType) {
                ...
                out.report.warnings.push_back(
                    Warning("one-of can not be a nested member for a type not "
                            "sub typed from object",
                        LogicalErrorWarning,
                        sourceMap));
