...
MarkdownNodeIterator SectionProcessor<mson::ValueMember>::processMSONSection(
    const MarkdownNodeIterator &node,
    ...
    // type section signatures are ineligible to be nested sections of a MSON
    // section
    ...
    if (sections.node.empty() ||
        (!sections.node.empty() &&
            sections.node.back().klass != mson::TypeSection::MemberTypeClass)) {
    ...
    if (pd.sectionContext() == MSONMixinSectionType) {
        ...
        if (!isSameBaseType(baseType, mixin.node.baseType)) {
            ...
            // WARN: Mixin base type should be compatible with the parent base
            // type
            ...
            sections.report.warnings.push_back(
                Warning("mixin base type should be the same as parent base "
                        "type. objects should contain object mixins. arrays "
        ...
        if ((baseType == mson::ValueBaseType ||
            ...
            if ((valueMember.node.valueDefinition.typeDefinition.baseType ==
                    ...
                    valueMember.node.valueDefinition.typeDefinition.baseType ==
                        mson::ObjectBaseType) &&
                ...
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                sections.report.warnings.push_back(
                    Warning("array member definition of type 'object' contains "
                            "value. You should use type definition without "
                            "value eg. '- (object)'",
                        LogicalErrorWarning,
                        sourceMap));
            }
            ...
            if ((propertyMember.node.valueDefinition.typeDefinition.baseType ==
                ...
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                sections.report.warnings.push_back(Warning(
                    "'object' with value definition. You should use type "
                    "definition without value eg. '- key (object)'",
                    LogicalErrorWarning,
                    sourceMap));
            }
            ...
            mdp::CharactersRangeSet sourceMap =
                mdp::BytesRangeSetToCharactersRangeSet(
                    node->sourceMap, pd.sourceCharacterIndex);
            sections.report.warnings.push_back(Warning(
                "sub-types of primitive types should not have nested members",
                LogicalErrorWarning,
                sourceMap));
        } else {
            ...
            mdp::CharactersRangeSet sourceMap =
                mdp::BytesRangeSetToCharactersRangeSet(
                    node->sourceMap, pd.sourceCharacterIndex);
            sections.report.warnings.push_back(Warning(
                "ignorning unrecognized block", IgnoringWarning, sourceMap));
