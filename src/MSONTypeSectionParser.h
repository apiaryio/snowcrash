    ...
    static MarkdownNodeIterator finalizeSignature(
        ...
        if (IEqual<std::string>()(signature.identifier, "Default")) {
            ...
            if (out.node.baseType != mson::ValueBaseType &&
                ...
                ss << "` not allowed for a type sub-typed from a primitive or "
                      "object type";
            ...
            if (out.node.baseType != mson::ObjectBaseType &&
                ...
                ss << "type section `" << signature.identifier;
                ss << "` is only allowed for a type sub-typed from an object "
                      "type";
        ...
        if (assignValues &&
            ...
            if (out.node.baseType == mson::PrimitiveBaseType ||
                ...
                // WARN: sample/default is for an object but it has values in
                // signature
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                out.report.warnings.push_back(
                    Warning("a sample and/or default type section for a type "
                            "which is sub-typed from an object cannot have "
                            "value(s) beside the keyword",
                        LogicalErrorWarning,
                        sourceMap));
            }
        }
