...
const char *const ResourceHeaderRegex =
...
const char *const NamedEndpointHeaderRegex =
    "^[[:blank:]]*" SYMBOL_IDENTIFIER "[[:blank:]]+\\[" HTTP_REQUEST_METHOD
    "[[:blank:]]+" URI_TEMPLATE "]$";
...
struct SectionProcessor<Resource> : public SectionProcessorBase<Resource>
{
    ...
    static MarkdownNodeIterator processNestedSection(
        ...
        case AttributesSectionType: {
            ...
            MarkdownNodeIterator cur =
                AttributesParser::parse(node, siblings, pd, attributes);
            ...
            if (!out.node.name.empty()) {
                ...
                if (SectionProcessor<DataStructureGroup>::isNamedTypeDuplicate(
                    ...
                    // Remove the attributes data from the AST since we are
                    // ignoring this
    ...
    static MarkdownNodeIterator processAction(const MarkdownNodeIterator &node,
        ...
        ActionIterator duplicate =
            SectionProcessor<Action>::findAction(out.node.actions, action.node);
    ...
    static MarkdownNodeIterator processModel(const MarkdownNodeIterator &node,
        ...
        if (!out.node.model.name.empty()) {
            ...
            ss << "' resource, a resource can be represented by a single model "
                  "only";
