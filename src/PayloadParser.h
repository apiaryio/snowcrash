...
const char *const RequestRegex =
    "^[[:blank:]]*[Rr]equest([[:blank:]]" SYMBOL_IDENTIFIER ")?" MEDIA_TYPE
    "?[[:blank:]]*";
...
struct SectionProcessor<Payload> : public SectionProcessorBase<Payload>
{
    ...
    static MarkdownNodeIterator processSignature(
        ...
        if (out.node.name.empty() &&
            ...
            out.report.warnings.push_back(Warning(
                "missing response HTTP status code, assuming 'Response 200'",
    ...
    static MarkdownNodeIterator processNestedSection(
        ...
        case AttributesSectionType: {
            ParseResultRef<Attributes> attributes(
                out.report, out.node.attributes, out.sourceMap.attributes);
        ...
        case ParametersSectionType: {
            if (pd.parentSectionContext() != RequestSectionType) {
                ...
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                out.report.warnings.push_back(
                    Warning("ignoring parameters section in a non request "
                            "payload section",
                        IgnoringWarning,
                        sourceMap));
            ...
            ParseResultRef<Parameters> parameters(
                out.report, out.node.parameters, out.sourceMap.parameters);
            ...
            if (!out.node.body.empty()) {
                ...
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                out.report.warnings.push_back(Warning(
                    "ignoring additional 'body' content, it is already defined",
                    RedefinitionWarning,
                    sourceMap));
            }
            ...
            if (!out.node.schema.empty()) {
                ...
                mdp::CharactersRangeSet sourceMap =
                    mdp::BytesRangeSetToCharactersRangeSet(
                        node->sourceMap, pd.sourceCharacterIndex);
                out.report.warnings.push_back(
                    Warning("ignoring additional 'schema' content, it is "
                            "already defined",
                        RedefinitionWarning,
                        sourceMap));
            }
    ...
    static SectionType getSectionType(
        ...
        case ModelPayloadSignature:
            return (nestedType != UndefinedSectionType) ? ModelSectionType
    ...
    static bool parseSignature(const MarkdownNodeIterator &node,
        ...
        if (RegexCapture(signature, regex, captureGroups, 5) &&
            ...
            if (!mediaType.empty()) {
                Header header =
                    std::make_pair(HTTPHeaderName::ContentType, mediaType);
    ...
    static void assingReferredPayload(
        ...
        if (isPayloadContentType && isModelContentType) {
            ...
            ss << "ignoring additional " << SectionName(pd.sectionContext())
               << " header(s), ";
            ss << "specify this header(s) in the referenced model definition "
                  "instead";
        ...
        if (out.node.body.empty() && out.node.attributes.empty() &&
            ...
            if (warnEmptyBody) {
                ...
                if (!contentLength.empty()) {
                    ...
                    ss << ", expected " << SectionName(BodySectionType)
                       << " for '" << transferEncoding << "' Transfer-Encoding";
                }
