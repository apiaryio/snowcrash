...
class ClassifiedExpression
{
  ...
  public:
    ...
    virtual bool IsExpressionType() const { return false; }
    ...
    bool IsSupportedExpressionType() { return isSupported; }
};
...
class PathStyleParameterExpansionExpression : public ClassifiedExpression
{
  public:
    PathStyleParameterExpansionExpression(const std::string &expression)
        : ClassifiedExpression(expression) {
        unsupportedWarningText =
            "URI template path style parameter expansion is not supported";
    }
...
class UndefinedExpression : public ClassifiedExpression
{
  public:
    ...
    bool IsExpressionType() const { return false; }
};
...
class URITemplateParser
{
  public:
    /**
    *  \brief Parse the URI template into scheme, host and path and then parse
    * for supported URI template expressions
    *
