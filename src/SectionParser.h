 ...
 */
template <typename T, typename Adapter> struct SectionParser
{
...
struct HeaderSectionAdapter
{
     ...
     * Markdown nodes on a nextStartingNode() call.
...
struct ListSectionAdapter
{
    ...
    static const MarkdownNodeIterator startingNode(
        ...
        for (auto it = seed->children().begin(); it != seed->children().end();
             ++it) {
