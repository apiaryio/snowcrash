...
const char *const ExpectedValuesContent = "nested list of possible parameter "
    ...
    : public SignatureSectionProcessorBase<Parameter>
{
    ...
    static SectionType sectionType(const MarkdownNodeIterator &node) {
        ...
        if (node->type == mdp::ListItemMarkdownNodeType &&
            ...
            subject =
                GetFirstLine(node->children().front().text, remainingContent);
            TrimString(subject);
            ...
            for (MarkdownNodeIterator it = node->children().begin();
                ...
                if (it->type == mdp::ListItemMarkdownNodeType &&
                    ...
                    if (RegexMatch(itSubject, MSONDefaultTypeSectionRegex) ||
                        ...
                        RegexMatch(
                            itSubject, MSONValueMembersTypeSectionRegex)) {
    ...
    static void parseAttributes(const mdp::MarkdownNodeIterator &node,
        ...
        size_t max =
            oldSyntax ? OLD_SYNTAX_MAX_ATTRIBUTES : NEW_SYNTAX_MAX_ATTRIBUTES;
