...
inline std::string &TrimStringEnd(std::string &s) {
    s.erase(std::find_if(s.rbegin(), s.rend(), std::not1(std::ptr_fun(isSpace)))
                .base(),
        s.end());
    return s;
}
...
inline std::string ReplaceString(
    const std::string &s, const std::string &find, const std::string &replace) {
    size_t pos = 0;
    std::string target(s);
...
template <typename T> struct Equal : std::binary_function<T, T, bool>
{
...
template <typename T> struct IEqual : std::binary_function<T, T, bool>
{
    bool operator()(const T &left, const T &right) const {
...
inline std::string ReconstructMarkdownHeader(const mdp::MarkdownNode &node) {
    ...
    // node.sourceMap - contain length, but we dont now count of space on both
    // sides
