 ...
 */
template <typename T> struct IntermediateParseResult
{
...
template <typename T> struct ParseResultRef
{
    ...
    ParseResultRef(ParseResult<T> &parseResult)
        : report(parseResult.report), node(parseResult.node),
 ...
 */
template <typename T> struct SectionProcessor;
 ...
 *  behavior.
 */
template <typename T> struct SectionProcessorBase
{
    ...
    static MarkdownNodeIterator processUnexpectedNode(
        ...
        if (node->type == mdp::HeaderMarkdownNodeType) {
            ss << "unexpected header block, expected a group, resource or an "
                  "action definition";
    ...
    static bool isDescriptionNode(
        ...
        if (std::find(upperTypes.begin(),
                upperTypes.end(),
                keywordSectionType) != upperTypes.end()) {
    ...
    static SectionTypes upperSectionTypes() { return SectionTypes(); }
 ...
 */
template <typename T> struct SectionProcessor : public SectionProcessorBase<T>
{};
}
