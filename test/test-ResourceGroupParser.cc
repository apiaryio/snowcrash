...
TEST_CASE("Parse canonical resource group", "[resource_group]") {
    ...
    REQUIRE(resourceGroup.node.content.elements()
                .at(1)
                .content.resource.uriTemplate == "/resource/{id}");
...
TEST_CASE("Parse resource group with empty resource", "[resource_group]") {
    mdp::ByteBuffer source = "# Group Name\n"
                             "p1\n"
                             "## /resource";
    ...
    REQUIRE(resourceGroup.node.content.elements()
                .at(1)
                .content.resource.uriTemplate == "/resource");
...
TEST_CASE("Parse multiple resource in anonymous group", "[resource_group]") {
    ...
    REQUIRE(resourceGroup.node.content.elements()
                .at(0)
                .content.resource.uriTemplate == "/r1");
    REQUIRE(resourceGroup.node.content.elements()
                .at(0)
                .content.resource.description == "p1");

    REQUIRE(resourceGroup.node.content.elements().at(1).element ==
            Element::ResourceElement);
    REQUIRE(resourceGroup.node.content.elements()
                .at(1)
                .content.resource.uriTemplate == "/r2");
    REQUIRE(resourceGroup.node.content.elements()
                .at(1)
                .content.resource.description == "p2");
...
TEST_CASE("Make sure method followed by a group does not eat the group",
    "[resource_group]") {
    mdp::ByteBuffer source = "# Group One\n"
                             "## /1\n"
                             "### POST\n"
                             "# Group Two\n";
...
TEST_CASE("Resource followed by a complete action",
    ...
    mdp::ByteBuffer source = "# Resource [/A]\n"
                             "# POST /B\n"
                             "+ Response 201\n";
...
TEST_CASE("Too eager complete action processing",
    "[resource_group][regression][187]") {
