...
TEST_CASE("Parse attributes with nested members", "[attributes]")
{
    ...
    REQUIRE(attributes.node.source.base.baseType == mson::ImplicitPropertyBaseType);
    REQUIRE(attributes.node.source.sections.size() == 1);
    REQUIRE(attributes.node.source.sections[0].type == mson::MemberTypeSectionType);
...
TEST_CASE("Parse attributes with block description", "[attributes]")
{
    ...
    REQUIRE(attributes.node.source.sections[0].type == mson::BlockDescriptionTypeSectionType);
    REQUIRE(attributes.node.source.sections[0].content.description == "Awesome description\n\n+ With list\n");
    REQUIRE(attributes.node.source.sections[1].type == mson::MemberTypeSectionType);
