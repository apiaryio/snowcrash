...
TEST_CASE("Parse canonical attributes", "[attributes]") {
    ...
    REQUIRE(attributes.node.typeDefinition.typeSpecification.name.base ==
            mson::ArrayTypeName);
    REQUIRE(
        attributes.node.typeDefinition.typeSpecification.nestedTypes.size() ==
        1);
...
TEST_CASE("Parse attributes with nested members", "[attributes]") {
    ...
    REQUIRE(attributes.sourceMap.sections.collection[0]
                .elements()
                .collection.size() == 2);
...
TEST_CASE("Parse attributes with block description", "[attributes]") {
    mdp::ByteBuffer source = "+ Attributes\n"
                             "    Awesome description\n\n"
                             "    + With list\n"
                             "    + Properties\n"
                             "        + message (string)";
    ...
    SourceMapHelper::check(
        ...
        12,
        3);
    REQUIRE(attributes.sourceMap.sections.collection[1]
                .elements()
                .collection.size() == 1);
}

TEST_CASE("Parse type attributes on attributes section", "[attributes]") {
    mdp::ByteBuffer source = "+ Attributes (fixed)\n"
                             "    + a: b";
