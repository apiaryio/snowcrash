...
TEST_CASE("resourcepparser/parse", "Parse resource")
{
    ...
    markdown.blocks.push_back(MarkdownBlock(HeaderBlockType, "GET", 1, MakeSourceDataBlock(2, 1)));
...
TEST_CASE("resourcepparser/parse-multi", "Parse mutliple methods")
{
    SourceData source = "0123";
    MarkdownBlock markdown;
    markdown.blocks.push_back(MarkdownBlock(HeaderBlockType, "/1", 1, MakeSourceDataBlock(0, 1)));    
    markdown.blocks.push_back(MarkdownBlock(HeaderBlockType, "GET", 1, MakeSourceDataBlock(0, 1)));
    markdown.blocks.push_back(MarkdownBlock(ParagraphBlockType, "p1", 0, MakeSourceDataBlock(1, 1)));
    markdown.blocks.push_back(MarkdownBlock(HeaderBlockType, "POST", 1, MakeSourceDataBlock(2, 1)));
