...
template <typename T, typename PARSER> struct SectionParserHelper
{
    ...
    static void parse(const mdp::ByteBuffer &source,
        ...
        pd.namedTypeDependencyTable.insert(namedTypes.dependencyTable.begin(),
...
struct SourceMapHelper
{
     ...
     * location & length.
     *
     * If 'nth' is given, check that particular row of the given sourceMap with
     * the
     ...
     */
    static void check(
        mdp::BytesRangeSet &sourceMap, int loc1, int len1, int loc2, int len2) {
