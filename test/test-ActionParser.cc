...
const mdp::ByteBuffer ActionFixture = "# My Method [GET]\n\n"
                                      "Method Description\n\n"
...
TEST_CASE("Parsing action", "[action]") {
    ...
    REQUIRE(
        action.sourceMap.examples.collection[0].requests.collection.size() ==
        0);
    REQUIRE(
        action.sourceMap.examples.collection[0].responses.collection.size() ==
        1);
...
TEST_CASE("Parse named action with () in title", "[action]") {
    mdp::ByteBuffer source = "# My Action (Deprecated) [GET]\n"
                             "+ Response 204\n";
...
TEST_CASE("Parse named action with path including () in title", "[action]") {
    mdp::ByteBuffer source = "# My Action (Deprecated) [GET /test]\n"
                             "+ Response 204\n";
...
TEST_CASE(
    "Parse method with multiple incomplete requests", "[action][blocks]") {
    mdp::ByteBuffer source = "# GET /1\n"
                             "+ Request A\n"
                             "+ Request B\n"
                             "  C\n"
                             "+ Response 200\n";
...
TEST_CASE("Give a warning when 2xx CONNECT has a body", "[action]") {
    mdp::ByteBuffer source = "# CONNECT /1\n"
                             "+ Response 201\n\n"
                             "        {}\n\n";
...
TEST_CASE("Warn when request is not followed by a response", "[action]") {
    ...
    REQUIRE(
        ...
        1);
    REQUIRE(
        action.sourceMap.examples.collection[1].requests.collection.size() ==
        1);
    REQUIRE(
        action.sourceMap.examples.collection[1].responses.collection.size() ==
        0);
}
...
//    ActionSectionType, action, ExportSourcemapOption, Models(), NULL,
//    namedTypes);
...
TEST_CASE("Named Endpoints Edge Cases", "[named_endpoint]") {
    const mdp::ByteBuffer source = "# Endpoint 1  [GET /e1]\n\n"
                                   "+ Response 204\n\n"
                                   "# Endpoint 2  [GET /e1]\n\n"
                                   "+ Response 204\n\n"
                                   "# Endpoint 3  [POST /e1]\n\n"
...
TEST_CASE(
    ...
    const mdp::ByteBuffer source = "# A [GET /]\n"
                                   "+ Properties\n"
                                   "+ Response 204";
...
TEST_CASE("Miss leading slash in URI", "[action][350]") {
    ...
    REQUIRE(report.warnings.begin()->message == "URI path in 'A [GET {param}]' "
                                                "is not absolute, it should "
                                                "have a leading forward slash");
}

TEST_CASE("Detect invalid reference to URI Template parameters in Action",
    "[action]") {
    mdp::ByteBuffer source = "## List [GET /orders{?abc}]\n\n"
                             "+ Parameters\n"
                             "    + ab (string)\n"
                             "    + bc (string)\n"
                             "    + ac (string)\n\n"
