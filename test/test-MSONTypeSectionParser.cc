...
TEST_CASE("Parse array mson sample list type section", "[mson][type_section]") {
    ...
    REQUIRE(typeSection.node.content.elements()
                .at(0)
                .content.value.valueDefinition.values.size() == 1);
    REQUIRE(typeSection.node.content.elements()
                .at(0)
...
TEST_CASE("Parse mson sample list type section with values as para for values "
          "base type",
    "[mson][type_section]") {
    mdp::ByteBuffer source = "- Sample\n\n"
                             "    a\n"
...
TEST_CASE("Parse multi-line mson sample header type section",
    "[mson][type_section]") {
    mdp::ByteBuffer source = "# Sample\n"
                             "   red\n"
                             "     green\n"
...
TEST_CASE("Parse multi-line mson sample header type section with multiple "
          "nested nodes",
    "[mson][type_section]") {
    mdp::ByteBuffer source = "# Sample\n"
                             "   red\n"
...
TEST_CASE("Parse mson properties list type section for values base type",
    "[mson][type_section]") {
    mdp::ByteBuffer source = "- Properties\n"
                             "    - first_name\n"
                             "    - last_name";
...
TEST_CASE("Parse mson sample type section for a simple object",
    ...
    REQUIRE(typeSection.node.content.elements()
                .at(0)
                .content.property.name.literal == "user");
    REQUIRE(typeSection.node.content.elements()
                .at(0)
                .content.property.sections.size() == 1);
    REQUIRE(typeSection.node.content.elements()
                .at(0)
                .content.property.sections[0]
                .klass == mson::TypeSection::MemberTypeClass);
    REQUIRE(typeSection.node.content.elements()
                .at(0)
                .content.property.sections[0]
                .content.elements()
    ...
    REQUIRE(typeSection.sourceMap.elements()
                .collection[0]
                .property.sections.collection.size() == 1);
    REQUIRE(typeSection.sourceMap.elements()
                .collection[0]
