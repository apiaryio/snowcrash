...
TEST_CASE("recognize headers signature", "[headers]") {
    ...
    SectionType sectionType =
        SectionProcessor<Headers>::sectionType(markdownAST.children().begin());
...
TEST_CASE("Headers parses should return warning on multiple definition of same "
          "headers",
    "[headers][issue][75]") {
    const mdp::ByteBuffer source = "+ Headers\n"
                                   "\n"
                                   "        Content-Type: application/json\n"
                                   "        Content-Type: application/json\n";
...
TEST_CASE("Parse header section with more same headers Set-Cookie and Link - "
          "there should not be warning",
    "[headers][issue][75]") {
    const mdp::ByteBuffer source = "+ Headers\n"
                                   "\n"
                                   "        Set-Cookie: abcd\n"
...
TEST_CASE(
    ...
    REQUIRE(headers.report.error.code == Error::OK);
    REQUIRE(
        headers.report.warnings.size() ==
        1); // one warning - multiple definitions w/ different case sensitivity
...
TEST_CASE("Aditional test for Header name insenstivity combined with allowed "
          "multiple headers",
    "[headers][issue][230]") {
    const mdp::ByteBuffer source = "+ Headers\n"
                                   "\n"
...
TEST_CASE("Skip completely invalid headers", "[headers][drafter-issue][382]") {
    ...
    SECTION("Invalid characters in header name") {
        ...
        REQUIRE(headers.report.warnings.size() ==
                1); // warning - header name is not defined correctly
        REQUIRE(headers.report.warnings[0].message ==
                "HTTP header name '<Header>' contains illegal character '<' "
                "(0x3c) skipping the header");
    ...
    SECTION("Invalid header") {
        ...
        REQUIRE(headers.report.warnings.size() ==
                2); // warning - header name is not defined correctly
        REQUIRE(headers.report.warnings[0].message ==
                "unable to parse HTTP header, expected '<header name> : "
...
TEST_CASE(
    "Parse headers with in the middle codefences", "[headers][codefence]") {
    const mdp::ByteBuffer source = "+ Headers\n"
                                   "        \n"
                                   "        a: b\n"
                                   "        ```\n"
