...
mdp::ByteBuffer BlueprintFixture = "meta: verse\n\n"
                                   "# Snowcrash API\n\n"
                                   "## Character\n"
                                   "Uncle Enzo\n\n"
                                   "# Group First\n"
...
TEST_CASE("Test required blueprint name on blueprint that starts with metadata",
    "[blueprint]") {
    mdp::ByteBuffer source = "meta: data\n"
                             "foo:bar\n\n"
                             "Hello";
...
TEST_CASE(
    ...
    mdp::ByteBuffer source = "# Group Posts\n"
                             "## /posts";
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(0)
                .content.resource.uriTemplate == "/posts");
...
TEST_CASE(
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(0)
                .content.resource.actions.size() == 2);
...
TEST_CASE("Parsing blueprint with mson data structures", "[blueprint]") {
    mdp::ByteBuffer source = "# Group Plans\n"
                             "\n"
                             "## Plans [/plans]\n"
                             "\n"
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(0)
                .content.dataStructure.name.symbol.literal == "Plan Base");
    REQUIRE(blueprint.node.content.elements()
                .at(1)
                .content.elements()
                .at(1)
                .content.dataStructure.name.symbol.literal == "Timestamp");
}

TEST_CASE("Parse blueprint with two named types having the same name",
    "[blueprint]") {
...
TEST_CASE("Parser blueprint correctly when having a big chain of inheritance "
          "in data structures",
    "[blueprint]") {
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(0)
                .element == Element::DataStructureElement);
    REQUIRE(blueprint.node.content.elements()
                .at(1)
                .content.elements()
                .at(0)
                .content.dataStructure.name.symbol.literal == "Timestamps");
    REQUIRE(blueprint.node.content.elements()
                .at(1)
                .content.elements()
                .at(1)
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(1)
                .content.dataStructure.name.symbol.literal == "Coupon Base");
    REQUIRE(blueprint.node.content.elements()
                .at(1)
                .content.elements()
                .at(2)
    ...
    REQUIRE(blueprint.node.content.elements()
                ...
                .content.elements()
                .at(2)
                .content.dataStructure.name.symbol.literal == "Coupon A");
}

TEST_CASE("Report error when coming across a super type reference to non "
          "existent named type",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
...
TEST_CASE("Report error when named type inherits a sub type in array",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
                             "## A (array[B])\n"
...
TEST_CASE("Report error when data Structure inheritance graph with only a few "
          "of them forming a cycle",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
                             "## A (B)\n"
...
TEST_CASE(
    ...
    REQUIRE(idC.valueDefinition.typeDefinition.baseType ==
            mson::ImplicitObjectBaseType);
    REQUIRE(idC.valueDefinition.typeDefinition.typeSpecification.name.symbol
                .literal == "A");
    ...
    REQUIRE(idA.valueDefinition.typeDefinition.baseType ==
            mson::ImplicitObjectBaseType);
    REQUIRE(idA.valueDefinition.typeDefinition.typeSpecification.name.symbol
                .literal == "B");
}

TEST_CASE("Do not report error when named sub type is referenced in nested "
          "members when reference happens first",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
                             "## B\n"
...
TEST_CASE("Do not report error when a resource attributes type is circularly "
          "referenced in nested members",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Post [/]\n"
                             "\n"
                             "+ Attributes (B)\n"
                             "    + id\n"
                             "\n"
...
TEST_CASE("Report error when named sub type is referenced as mixin when "
          "reference happens first",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
                             "## B\n"
...
TEST_CASE("Report error when a named type is defined twice, once with base "
          "type and other inheritance",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "\n"
                             "## A (B)\n"
...
TEST_CASE("Parse correctly when a resource named type is non-circularly "
          "referenced in action attributes",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Question [/question]\n"
                             "+ Attributes\n"
                             "    + question\n"
                             "\n"
                             "# Choice [/choice]\n"
...
TEST_CASE(
    "When an object member contains a mixin of array type", "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "## C\n"
                             "+ d (object)\n"
                             "    + Include A\n"
...
TEST_CASE(
    "When an array member contains a mixin of object type", "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "## C\n"
                             "+ d (array)\n"
                             "    + Include A\n"
...
TEST_CASE("Any named type data structure should be able to be overridden when "
          "referenced",
    "[blueprint]") {
    mdp::ByteBuffer source = "# Tesla\n"
                             "\n"
                             "# Data Structures\n"
                             "## User (object)\n"
                             "+ username: kyle\n"
...
TEST_CASE(
    "Parse blueprint with escaped datastructure reference", "[blueprint]") {
    mdp::ByteBuffer source = "# Data Structures\n"
                             "## Test\n"
                             "- name - test name\n"
                             "\n"
