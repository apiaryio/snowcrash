...
TEST_CASE("Parse canonical mson one of", "[mson][one_of]") {
    ...
    REQUIRE(oneOf.node.at(0)
                .content.property.valueDefinition.values.at(0)
                .literal == "Andhra Pradesh");
...
TEST_CASE("Parse mson one of with one of", "[mson][one_of]") {
    ...
    REQUIRE(
        oneOf.node.at(1).content.oneOf().at(0).content.property.name.literal ==
        "given_name");
    REQUIRE(oneOf.node.at(1)
                .content.oneOf()
                .at(0)
    ...
    REQUIRE(oneOf.node.at(1).content.oneOf().at(1).klass ==
            mson::Element::PropertyClass);
    REQUIRE(
        oneOf.node.at(1).content.oneOf().at(1).content.property.name.literal ==
        "suffixed_name");
    REQUIRE(oneOf.node.at(1)
                .content.oneOf()
                .at(1)
